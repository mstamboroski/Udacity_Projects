...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    dim = 2*(1 + num_landmarks)  #define a dimensao da matriz e do vetor, N eh o comprimento do caminho, vezes dois pois estou modelando x e y na mesma estrutura de dados

    #make the constraint information matrix and vector
    Omega = matrix()
    Omega.zero(dim,dim)
    Omega.value[0][0] = 1.0   #initial constrains with strenght 1.0, in the main diagonal, it means, one constrain for x and one for y
    Omega.value[1][1] = 1.0

    Xi = matrix()
    Xi.zero(dim,1)
    Xi.value[0][0] = world_size / 2.0 #initial position of the initial constrains
    ...
    for k in range(len(data)):
        ...
        for i in range(len(measurements)):
            ...
            for b in range(2):
                ...
                Xi.value[b][0] += -measurements[i][1+b] / measurement_noise
