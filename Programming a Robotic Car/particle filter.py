...
class robot:
    ...
    def measurement_prob(self, measurements):
        ...
        # compute errors
        error = 1.0
        for i in range(len(measurements)):  #percorro cada medida
            error_bearing = abs(measurements[i] - predicted_measurements[i]) #erro absoluto entre a leitura feita e a prevista
            error_bearing = (error_bearing + pi) % (2.0 * pi) - pi # truncate, para o mundo ciclico, dando um valor entre pi e -pi


            # update Gaussian
            # para cada measurements eu multiplico por uma gaussiana, e a ultima gaussiana, e a ultima gaussiana eh o peso do erro
    ...
    def move(self, motion, tolerance = 0.001): # Do not change the name of this function
        ...
        if abs(turn) < tolerance:  #para ver se meu turn fara alguma diferenca, senao aproximo por um movimento em linha reta
            ...
            cx = self.x - (sin(self.orientation) * radius)  #center of the circle i'm turning
    ...
    def sense(self, add_noise): #do not change the name of this function
        ...
        for i in range(len(landmarks)):
            #atan2 eh a funcao matematica para calcular o angulo, ela pegao valor y como primeiro argumento e o valor x como o segundo
            bearing=atan2(landmarks[i][0] - self.y,    # Isto eh o angulo local de uma landmark relativo a coordenada do robo
                          landmarks[i][1] - self.x) - self.orientation  #como o robo tem uma orientacao global, tenho que subtrarir para obter meu bearing
            if add_noise:  # se ha noise, produzo uma variavel de soma de noise
                bearing += random.gauss(0.0, self.bearing_noise)
            bearing %= 2.0*pi  # garantir que o bearing eh normalizado entre 0 e 2*pi
            Z.append(bearing)
...
def generate_ground_truth(motions):
    ...
    for t in range(T):
        ...
        Z.append(myrobot.sense(0))  # passando 1 para sense, assume-se noise no sensorioamento
    #print 'Robot:    ', myrobot
    return [myrobot, Z]
...
def particle_filter(motions, measurements, N=1500):
    # --------
    # N eh o numero de particulas criadas
    # Make particles
    #
